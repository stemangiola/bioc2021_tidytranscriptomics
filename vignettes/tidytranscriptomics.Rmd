...
We'll set up the airway data for our RNA-seq analysis. We'll create a column with shorter sample names and a column with gene symbols. We can get the gene symbols for these Ensembl gene ids using the Bioconductor annotation package for human, `org.Hs.eg.db`.
...
A powerful tool tidySingleCellExperiment enables us to use with our single-cell data is tidyverse's [nest](https://rstudio-education.github.io/tidyverse-cookbook/transform-tables.html#nest-a-data-frame). Nesting allows us to perform independent analyses on subsets of the data easily. For example, our data contains 10 single-cell datasets from different sources; we can nest by dataset and analyse the mitochondrial content for each.
