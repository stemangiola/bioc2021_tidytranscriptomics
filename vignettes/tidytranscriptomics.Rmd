...
We filter out lowly expressed genes and scale counts to normalise for technical and composition differences between the samples using tidybulk.
...
```{r}
pbmc %>% select(sample, cell, nCount_RNA)
```
...
```{r}
pbmc %>%
  mutate(sample_name=tolower(sample)) %>%
  # select columns to view    
  select(sample, sample_name)
```
...
Poll: which cell cluster has the higher median mitochondrion transcriptional percentage?
```
...
```{poll_10 class.source="poll"}
Poll: Is the variability explained by the first UMAP dimension when calculating 2 components (ncomponents = 2) equal/more/less than when calculating 3 components.?
```
