...
We'll set up the airway data for our RNA sequencing analysis. We'll create the column with shorter sample names and a column with gene symbols. We can get the gene symbols for these Ensembl gene ids using the Bioconductor annotation package for human, `org.Hs.eg.db`.
...
#### Long excercise on nesting

Using the nest examples above, can you create a column that includes the mitochondrial information?
...
mito_info_all_datasets = bioc2021tidytranscriptomics::mito_info_all_datasets 
```
...
#### Excercise about UMAP

Without creating any variable, check that the variability explained with the first UMAP dimension when calculating 2 components (ncomponents = 2) is equal/more/less from when calculating 3 components.
...
#### Exercise: use tidyverse grammar to identify the most abundant cell type
