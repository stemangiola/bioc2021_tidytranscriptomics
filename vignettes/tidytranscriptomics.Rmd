...
This workshop will present how to perform analysis of RNA sequencing data following the tidy data paradigm [@wickham2014tidy]. The tidy data paradigm provides a standard way to organise data values within a dataset, where each variable is a column, each observation is a row, and data is manipulated using an easy-to-understand vocabulary. Most importantly, the data structure remains consistent across manipulation and analysis functions. This can be achieved for RNA sequencing data with the [tidybulk](https://stemangiola.github.io/tidybulk/), [tidySummarizedExperiment](https://stemangiola.github.io/tidySummarizedExperiment/) and [tidySingleCellExperiment](https://stemangiola.github.io/tidySingleCellExperiment/) packages. These packages are part of the tidytranscriptomics suite that introduces a tidy approach to RNA sequencing data representation and analysis.
...
_Note: you should load the *tidybulk* and *tidySummarizedExperiment* libraries after the tidyverse core packages for best integration._
...
The airway data is a Bioconductor *RangedSummarizedExperiment* object, a type of *SummarizedExperiment* object. For more information on these objects see [here](https://bioconductor.org/packages/release/bioc/vignettes/SummarizedExperiment/inst/doc/SummarizedExperiment.html#anatomy-of-a-summarizedexperiment).

## tidySummarizedExperiment

The `tidySummarizedExperiment` package enables any *SummarizedExperiment* object to be displayed and manipulated with tidy principles, without affecting any *SummarizedExperiment* behaviour.

If we load *tidySummarizedExperiment* and then view the airway data, it now displays as a tibble. A tibble is the tidyverse table format.
...
Note that we can also interact with the tidy SummarizedExperiment object using commands for SummarizedExperiment objects.
...
We can use `group_by` and `summarise` to get the total counts for each sample.

```{r}
airway %>%
    group_by(sample, cell, dex) %>%
...
We'll use mutate to set up the data for our RNA-seq analysis. We'll create the column with shorter sample names and a column with gene symbols. We can get the gene symbols for these Ensembl gene ids using the Bioconductor annotation package for human, `org.Hs.eg.db`.
