...
The airway data is stored as a Bioconductor *RangedSummarizedExperiment* object.
...
In this format it is not so easy to see the information it contains. We will convert this object into a tidy SummarizedExperiment object.
...
```{r}
# convert to tidySummarizedExperiment object
airway <- tidy(airway)
```

It is still a SummarizedExperiment object so we can interact with it using SummarizedExperiment commands.

```{r}
assays(airway)
```

But it now displays like a tibble, a tidyverse table.

```{r}
airway
```

Now we can see that the airway counts object contains information about genes and samples. The first column has the Ensembl gene identifier, the second column has the sample identifier and the third column has the gene transcription abundance expressed in number of reads aligning to the gene in each experimental sample. The remaining columns include sample-wise information. The dex column tells us whether the samples are treated or untreated and the cell column tells us what cell line they are from.

## Tidyverse commands

We can use tidyverse commands to explore the tidy SummarizedExperiment object. In this workshop we will be using the tidyverse pipe `%>%`. This 'pipes' the output from the command on the left into the command on the right/below. Using the pipe is not essential but it reduces the amount of code we need to write when we have multiple steps (as we'll see later). It also can make the steps clearer and easier to see. For more details on the pipe see [here](https://r4ds.had.co.nz/pipes.html).

We can use `filter` to choose rows.
...
We could use `group_by` and `summarise` to get the total counts for each sample.
...
We convert to a tidybulk tibble to perform the RNA-seq analysis. We will use mutate to create a new column called sample_name that shortens the sample names. We will also use mutate to add a column containing gene symbols.
...
```{r}
BRCA <- bioc2021tidytranscriptomics::BRCA %>% tidybulk()
BRCA
```
...
BRCA_cell_type <-
  BRCA %>%
  deconvolve_cellularity(method="cibersort", cores = 1)
